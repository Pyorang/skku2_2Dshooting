...
| 정적 필드 | `s\_` | `s\_defaultLogger` | 이름은 camelCase 사용 |

| 스레드 정적 필드 | `t\_` | `t\_timeSpan` | 이름은 camelCase 사용 |
...
\* \*\*밑줄 연속 사용:\*\* 두 개의 연속된 밑줄(`\_\_`)은 사용하지 않습니다.
...
\* \*\*주석 (코드):\*\* \*\*`//`\*\*를 사용하며, 코드 줄 끝이 아닌 \*\*별도의 줄\*\*에 배치합니다. 주석 텍스트는 \*\*대문자로 시작\*\*하고 끝에 \*\*마침표\*\*를 붙입니다.
